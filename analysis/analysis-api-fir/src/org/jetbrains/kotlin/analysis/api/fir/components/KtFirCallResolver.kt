...
internal class KtFirCallResolver(
    ...
    private fun FirDelegatedConstructorCall.collectCallCandidatesForDelegatedConstructorCall(
        ...
        val derivedClassPsi = when (psi) {
            is KtConstructorDelegationCall -> (psi.parent as? KtSecondaryConstructor)?.parent as? KtClassOrObject
            is KtSuperTypeCallEntry -> (psi.parent as? KtSuperTypeList)?.parent as? KtClassOrObject
            else -> null
        }

        val derivedClass = derivedClassPsi?.getOrBuildFirSafe<FirClass>(firResolveSession) ?: return emptyList()
