...
context(Fir2IrComponents)
internal tailrec fun FirCallableSymbol<*>.unwrapCallRepresentative(): FirCallableSymbol<*> {
    val fir = fir
    if (fir is FirConstructor) {
        val originalForTypeAlias = fir.originalConstructorIfTypeAlias
        if (originalForTypeAlias != null) {
            return originalForTypeAlias.symbol.unwrapCallRepresentative()
        }
    }
    ...
    if (fir.isIntersectionOverride) {
        ...
        if (fir.dispatchReceiverType is ConeIntersectionType) {
            return fir.baseForIntersectionOverride!!.symbol.unwrapCallRepresentative()
        }

        return this
    }

    val originalForSubstitution = fir.originalForSubstitutionOverride?.symbol ?: return this
    return originalForSubstitution.unwrapCallRepresentative()
}
